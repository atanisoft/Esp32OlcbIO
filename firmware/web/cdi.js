...
function refresh_all_cdi_fields() {
    ...
    toggleModal('#node-cdi-pending');
    toggleModal('#node-cfg');
...
async function download_cdi_fields() {
    const maxConcurrentWorkers = $('#max_cdi_workers').val();
    var activeWorkers = 0;
    var fieldIndex = 0;
...
class CdiField extends CdiBaseElement {
    ...
    get render() {
        ...
        if (this.#mapped_values.length) {
            content +=
                String.format('<div class="column column-3"><select class="form-select" id="{0}" data-offset="{1}" data-type="{2}" data-size="{3}" data-node="{4}" data-space="{5}" oninput="enable_button(\'save-{0}\')">',
